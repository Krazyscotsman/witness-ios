...
// MARK: - Conversation history (Ask Scarlett memory-scoped + Talk whole-life). Read-only text transcripts.
// Endpoints (/api/v1/jarvis, Bearer):
//   memory list:  GET /memories/{memory_id}/conversations   → { conversations: [MemoryConvSummary] }
//   talk list:    GET /conversations/recent?limit=50         → { conversations: [RecentConvSummary] } (MIXED)
//   transcript:   GET /conversations/{id}/turns              → { turns: [ConversationTurn] } (shared)
// No audio/TTS, no composer — purely a read-only review of past turns.
...
struct ConversationHistoryView: View {
    ...
    private var loading: some View {
        VStack(spacing: 12) { ProgressView().tint(WV.teal); Text("Loading conversations…").font(.system(size: 14)).foregroundStyle(WT.ink.opacity(0.55)) }
...
struct TranscriptView: View {
    ...
    var body: some View {
        ZStack {
            ...
            case .empty:
                Text("This conversation has no messages.").font(.system(size: 14)).foregroundStyle(WT.ink.opacity(0.55)).padding()
            case .failed(let m):
                VStack(spacing: 12) {
                    Text(m).font(.system(size: 14)).foregroundStyle(WT.ink.opacity(0.7)).multilineTextAlignment(.center).fixedSize(horizontal: false, vertical: true)
                    Button { Task { await vm.load(conversationId: conversationId, auth: auth) } } label: { Text("Try again").font(.system(size: 15, weight: .medium)).foregroundStyle(WV.teal) }.witnessPress()
                }.padding(40)
            case .loaded:
                ScrollView(showsIndicators: false) {
                    VStack(spacing: 14) {
