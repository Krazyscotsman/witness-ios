# Witness — Anchors screen Phase 1 (backend-wired list + detail + inert delete) — Proposal

Status: **PROPOSED — nothing applied, no build, no git.** Awaiting review/approval.

## Read-first findings
- Entry: Insights tab → InsightsView → `.navigationDestination(for: InsightItem.self)` → `case "anchors": AnchorsView()`.
  InsightsView takes only `@Binding var path` (the lifted insightsPath); no `auth`.
- Current AnchorsView (+ AnchorsModel) is a LOCAL "Truth Registry": category-based, search, Critical People,
  create/edit forms, `AnchorStore` persisted to UserDefaults. NOT wired to the backend /entities API.
  Referenced only from InsightsView.
- Memory detail reuse: MemoriesView pushes `MemoryDetailView(listItem: MemoryDTO, auth:)`. MemoryDetailView
  needs listItem only for the id (fetch /detail) + instant header (title/date) + auth; works from any stack.
  A linked memory can open it via a MemoryDTO built from {id, title, date}, pushed on the Insights stack.

## Decisions (baked in; change any)
1. New `AnchorsListView` becomes Insights→Anchors; the local Truth Registry (AnchorsView/AnchorsModel/forms)
   is LEFT in place but unreferenced (kept, not deleted). Recommend.
2. Reuse MemoryDetailView via a MemoryDTO built from LinkedMemory (guard missing id).
3. Thread auth: MainTabView → InsightsView → AnchorsListView → AnchorEntityDetailView → MemoryDetailView.
4. Delete = inert: destructive button + confirm dialog → single `deleteAnchor()` no-op stub (seam) + a subtle
   "not available yet" note. Never calls DELETE /entities/{id} or DELETE /timeline/{category}/{id}.
5. New type names avoid collisions: `AnchorsListView` / `AnchorEntityDetailView` (existing AnchorsView /
   AnchorDetailView untouched).

---

## APIModels.swift — entity models (append)
```swift
...
## New file: AnchorsListView.swift
```swift
import SwiftUI
import Combine
...
## InsightsView.swift — take auth + repoint anchors
```diff
...
```
(The local Truth Registry `AnchorsView` becomes unreferenced — kept in the codebase, not deleted.)

## MainTabView.swift — pass auth to InsightsView
```diff
...
## After approval
Apply; build 0/0 + per-file diagnostics. Honest note: the live /entities + /entities/{id} decode, the
client-side is_anchor filter, and linked-memory → MemoryDetailView hop are wired to the verified contract but
not yet exercised against the backend (device pass recommended). Delete stays inert. No git.
